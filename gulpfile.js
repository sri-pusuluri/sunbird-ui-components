...
function stylesFabricator() {
  let fCss;
  let fExternalCss;
  fExternalCss = gulp.src('node_modules/codecopy/umd/codecopy.min.css');
  fCss = gulp
  ...
  return merge(fCss, fExternalCss)
    .pipe(concat('f.css'))
...
function stylesToolkit() {
  let toolkitCss;
  let toolkitExternalCss;
  toolkitExternalCss = gulp.src(
    'lib/sunbird-ui-component/dist/sb-components.css',
    'node_modules/codecopy/umd/codecopy.min.css'
  );
  toolkitCss = gulp.src(config.styles.toolkit.src)
  ...
  return merge(toolkitCss, toolkitExternalCss)
